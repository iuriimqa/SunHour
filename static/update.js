...
async function fetchWeather(date){
    try{
        ...
        const response2 = await fetch(`http://api.weatherapi.com/v1/current.json?key=4fd8239d12344aceb8081603230106&q=${cityx}&dt=${date}&aqi=no`)
        ...
        feels.textContent = `Feels like: ${data2['current'].feelslike_c}°`
        rain.textContent = `Humidity is: ${data2['current'].humidity}%`
